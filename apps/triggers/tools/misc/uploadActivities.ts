...
const projectActionUrl = (projectId: string): string =>
  `${baseUrl}/projects/${projectId}/actions`;
...
const getUsers = async (
  config: ApiAuth,
): Promise<(NameIds & { email: string; phone: string })[]> => {
...
const updateIdInGSheet = async (
  ...
  for (const row of rows) {
    ...
    if (row.get('phase')) {
      const phaseId = phaseIds.find(
        (phase) => phase.name === row.get('phase'),
      )?.uuid;
      row.set('phaseId', phaseId);
    }
    if (row.get('category')) {
      const categoryId = categoryIds.find(
        (category) => category.name === row.get('category'),
      )?.uuid;
    ...
    if (row.get('responsible')) {
      ...
      row.set(
        'responsibleId',
        user?.uuid || '❌ NO USER ID FOUND. PLEASE CHECK THE EMAIL',
      );
      row.set(
        'responsibleName',
        user?.name || '❌ NO USER NAME FOUND. PLEASE CHECK THE EMAIL',
      );
