import {
  SearchPaginateFunction,
  SearchPaginateOptions,
} from '../types/pagination.types';

export const searchPaginator = (
  defaultOptions: SearchPaginateOptions,
): SearchPaginateFunction => {
  return async <T>(prisma: any, modelName: string, options: any) => {
    ...
    const lastPage = Math.ceil(
      total / (options?.perPage || defaultOptions.perPage),
    );
    ...
    return {
      ...
      meta: {
        ...
        prev:
          options?.page > 1 ? (options?.page || defaultOptions.page) - 1 : null,
        next:
          options?.page < lastPage
            ? (options?.page || defaultOptions.page) + 1
            : null,
      },
    };
  };
